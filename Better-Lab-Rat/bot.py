...
def staff_check():
    async def predicate(ctx):
        ...
        if staff_role is None:
            return False
        else: # While arguably you don't need this else, it makes the code more readable to have it
            return True
    ...
    for counter in range(0, 15):
        try:
            ...
            if user is None:
                ...
                level        = levels[counter][0]
            leaderboard.append(f"**#{counter+1} {user_mention}: Messages: {level}**")
        except IndexError:
            break # No more users in list, no need to keep checking
...
async def lock(ctx, time: int=None):
    await ctx.message.delete() # Delete issued command
    ...
    if time == None:
        ...
        await asyncio.sleep(time*60) # Could use some sort of schedualer rather than the current implementation that holds a thread open
...
async def kick(ctx, member: discord.Member, *, reason: str):
    ...
    except discord.Forbidden: # User Privacy Too High
        pass 
...
async def ban(ctx, member: discord.Member, *, reason: str):
    ...
    user_embed = discord.Embed(title="Banned",
                               ...
                               color=0xFF5555)
    await ctx.send(embed=embed)
    try discord.Forbidden: # User Privacy Too High
        await member.send(embed=user_embed)
    except:
        pass
